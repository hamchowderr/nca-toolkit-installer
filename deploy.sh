...
WEBHOOK_URL="https://build.otakusolutions.io/webhook-test/ncat-installer"

# Track failures
trap 'curl -s -X POST "$WEBHOOK_URL" -H "Content-Type: application/json" -d "{\"email\": \"${ACCOUNT:-unknown}\", \"success\": false}" > /dev/null 2>&1 || true' ERR
...
curl -s -X POST "$WEBHOOK_URL" -H "Content-Type: application/json" -d "{\"email\": \"${ACCOUNT}\", \"success\": true}" > /dev/null 2>&1 || true
